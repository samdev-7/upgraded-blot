...
An upgrade for [Blot](https://github.com/hackclub/blot). Hack Club's 2-axis pen plotter.
...
4. File → Open → `rmrrf/firmware/firmware.ino`. The IDE picks up the other `.cpp`/`.h` files in that folder automatically.
...
```bash
chmod +x rmrrf/ui/blot.sh       # first run only
./rmrrf/ui/blot.sh
```
